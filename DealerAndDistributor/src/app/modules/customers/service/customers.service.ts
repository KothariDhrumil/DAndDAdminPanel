...
import { CUSTOMERS_WITH_TENANTS_API, CUSTOMERS_BY_TENANT_API, CUSTOMERS_SEARCH_BY_PHONE_API, CUSTOMERS_LINK_API, CUSTOMERS_API, CUSTOMERS_TENANT_PROFILE_API, CUSTOMERS_CHILD_API, CUSTOMERS_CHILD_LINK_API } from '../../../core/helpers/routes/api-endpoints';
...
export class CustomersService {
  ...
  getCustomersByTenant(pageNumber: number, pageSize: number): Observable<PaginatedApiResponse<CustomerWithTenants[]>> {
    const params = new HttpParams().set('pageNumber', pageNumber).set('pageSize', pageSize);
    return this.http.get<PaginatedApiResponse<CustomerWithTenants[]>>(CUSTOMERS_BY_TENANT_API, { params });
  }
