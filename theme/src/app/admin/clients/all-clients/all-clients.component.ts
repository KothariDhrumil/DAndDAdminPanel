...
export class AllclientComponent implements OnInit, OnDestroy {
  columnDefinitions = [
    ...
    { def: 'name', label: 'Client Name', type: 'text', visible: true },
    { def: 'mobile', label: 'Mobile', type: 'phone', visible: true },
    { def: 'email', label: 'Email', type: 'email', visible: true },
    { def: 'company_name', label: 'Company Name', type: 'text', visible: true },
    { def: 'currency', label: 'Currency', type: 'text', visible: true },
    {
      def: 'billing_method',
  ...
  exportExcel() {
    const exportData = this.dataSource.filteredData.map((x) => ({
      ...
      'Contact Person': x.contact_person,
      'Contact Phone': x.contact_phone,
      Status: x.status,
      'Contract Document': x.contract_document,
      Address: x.address,
      Notes: x.notes, // Optional if needed
    }));

    TableExportUtil.exportToExcel(exportData, 'client_contracts_export');
  }
