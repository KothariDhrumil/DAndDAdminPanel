...
namespace Example6.MvcWebApp.Sharding.Controllers
{
    public class TenantController : Controller
    {
        ...
        {
            var status = await _authTenantAdmin.AddHierarchicalTenantAsync(input.TenantName, input.ParentId,null,
                hasOwnDb: input.HasOwnDb, input.ShardingName);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        [HasPermission(Example6Permissions.TenantMove)]
        public async Task<IActionResult> Move(HybridShardingTenantDto input)
        {
